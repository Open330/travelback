...
- Work: update compatible direct dependencies to registry-current stable releases verified on 2026-07-16, including React 19.2.7, MapLibre GL 5.24.0, Lucide React 1.24.0, Playwright 1.61.1, Tailwind 4.3.2, ESLint 10.7.0, and TypeScript 7.0.2; align `@types/node` to the current Node 24 line rather than the unrelated Node 26 line.
...
- Status: Scheduled. The `korean-naturalizer` skill must be read and announced before editing the Korean phrase.
...
- Status: Scheduled after its prerequisite code changes.
...
No warning or failure may be silently ignored. Repairs discovered only by a gate are counted separately from the 19 review-plan items. Deployment remains out of scope.
